...
interface CurrentPriceCardProps {
  ...
  onTokenChange: (newIdx: number) => void
}
 ...
 *
 * @param props
 * @param props.pool - Pool entity with token metadata and TVL snapshots
 * @param props.selectedTokenIdx - Active token for price display (0 or 1)
 * @param props.onTokenChange - Callback to sync PoolCharts toggle state
 */
export function CurrentPriceCard({
  pool,
  selectedTokenIdx,
  onTokenChange
}: CurrentPriceCardProps): ReactNode {
