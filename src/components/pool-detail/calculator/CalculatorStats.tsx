...
function PlaceholderStats({ isPulsing = false }: { isPulsing?: boolean }): ReactNode {
 ...
 *
 * @param props
 * @param props.results - Simulation output from simulateRangePerformance
 * @param props.results.dailyFeesUSD - Projected 24h revenue
 * @param props.results.APR - Annualized percentage rate
 * @param props.results.daysOfData - Sample size for calculation (affects confidence)
 * @param props.isLoading - Active computation state
 * @param props.fetchError - GraphQL/network failure message
 * @param props.poolData - Pool metadata (prices, symbols, volume)
 * @param props.rangeInputs - User-defined range boundaries and capital
 */
export function CalculatorStats({
  ...
  poolData,
  rangeInputs
}: CalculatorStatsProps): ReactNode {
